...
export default function AgendaDay(props: AgendaDayProps) {
  ...
  if (events && events.length > 0) {
    events.forEach((evt, i) => {
      ...
      if (showVipAlert) {
        alertIcon = (
          <FaExclamationTriangle
            className="agenda-day-event-alert"
      ...
      if (
        currentReportSelection.expandedRow &&
        currentReportSelection.expandedEvent &&
        currentReportSelection.expandedRow === evt.externalEventId
      ) {
        eventRows.push(<EventDataExpanded key={`exp_${evt.externalEventId}`} />);
      }
    });
  }
  ...
  const displayDate = agendaDate
    ? `${agendaDate.format('ddd')} ${agendaDate.format('MM/DD/YYYY')}`
    : '';
    ...
    <Row key={`ad_${key}`} className="agenda-day-row">
      <Col className="agenda-day">
        <div
          ...
          className="agenda-day-name"
        >
          {agendaDate?.format('dddd, MMMM DD, YYYY')}
        </div>
        {noteRows}
        {eventRows}
        <AddNoteModal
