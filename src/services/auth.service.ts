...
export class AuthService {
  ...
  resetPasswordSecure = async (username: string, password: string, confirmPassword: string): Promise<UserResponse> => {
    ...
    return this.instance
      ...
      .catch((err) => {
        ...
        if (err?.response?.data?.msg) {
          ...
          errorMessage = "Unknown error while resetting password - please contact your administrator";
        }
    ...
    return this.instance
      ...
      .catch((err) => {
        ...
        if (err?.response?.data?.msg) {
          ...
          errorMessage = "Unknown error during send of password reset email - please contact your administrator";
        }
        userResponse.errorMessage = errorMessage;
        return userResponse;
      });
    ...
    return this.instance
      ...
      .catch((err) => {
        ...
        if (err?.response?.data?.msg) {
          ...
          errorMessage = "Unknown error while validating reset code - please contact your administrator";
        }
        userResponse.errorMessage = errorMessage;
        return userResponse;
      });
  ...
  resetPassword = async (username: string, password: string, confirmPassword: string, code: number): Promise<UserResponse> => {
    const headers = {
      'Content-Type': 'application/json',
      'x-api-key': `${process.env.NEXT_PUBLIC_USER_API_KEY}`
    };
