            ...
            if (order.isActive && !order.isDeleted) {
                order.tickets?.forEach((ticket) => {
                    if (ticket.isActive) {
                        ...
                        if (!eventHasTicketTypes && !ticketTypes.find(x => x.ticketTypeName.toLowerCase() == ticket.ticketType.toLowerCase())) {
                            ticketTypes.push({
                        ...
                        if (!collection) {
                            ...
                            const indexToUpdate = collection.findIndex(item => item.TicketType.toLowerCase() === ticket.ticketType.toLowerCase());
                            if (indexToUpdate >= 0) {
