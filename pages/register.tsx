        ...
        register(username, firstName, lastName, sellerId, password, confirmPassword, notes)
            .then((response) => {
                if (response.errorMessage) {
                    ...
                    setRegisterSuccess('User registration succeeded, please wait for response from the administrator');
                }
            })
            .catch ((e) => {
                console.log(e);
    ...
    return (
        <Container className="wrapper">
            ...
            <Row>
                <Col>
                    <div className="form-group">
                        <Button onClick={onSubmit}>Submit</Button><Button onClick={onBack}>Back</Button>
                        {registerError ? 
                        <div className="h-fit flex flex-row gap-2 items-center justify-center danger">{registerError}</div> :
                        ''
                        }
                        {registerSuccess ? 
                        <div className="h-fit flex flex-row gap-2 items-center justify-center success">{registerSuccess}</div> :
                        ''
                        }
                    </div>
                    </Col>
