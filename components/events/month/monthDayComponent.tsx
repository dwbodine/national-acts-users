...
export default function MonthDay(props: any) {
    ...
    const editNewNoteAndMarkComplete = () => {
        if (!monthDate || !displayNoteText || !displayNoteTitle || noteId == 0) {
            return;
        }
    ...
    const displayDate = monthDate ? `${monthDate.format('ddd')} ${monthDate.format('MM/DD/YYYY')}` : '';

    return (
        <div key={`md_${key}`}>
            <div title={`Add a note for ${displayDate}`} onClick={handleNotesOpen} className="month-day-name">{monthDate?.format('ddd')}</div>
