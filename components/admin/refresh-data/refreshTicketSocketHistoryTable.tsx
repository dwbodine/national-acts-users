    ...
    return(
        ...
        <Table height={420} data={results} bordered cellBordered>
            ...
            <Column>
                ...
                <Cell>{rowData => (rowData.serviceEventsSkipped && rowData.serviceEventsSkipped.length > 0) ? 'yes' : 'no'}</Cell>
            </Column>
            <Column>
                <HeaderCell>Events failed</HeaderCell>
                <Cell>{rowData => (rowData.eventsFailed && rowData.eventsFailed.length > 0) ? 'yes' : 'no'}</Cell>
            </Column>
            <Column>
                <HeaderCell>Orders failed</HeaderCell>
                <Cell>{rowData => (rowData.ordersFailed && rowData.ordersFailed.length > 0) ? 'yes' : 'no'}</Cell>
            </Column>
            <Column>
                <HeaderCell>Tickets failed</HeaderCell>
                <Cell>{rowData => (rowData.ticketsFailed && rowData.ticketsFailed.length > 0) ? 'yes' : 'no'}</Cell>
            </Column>
            <Column>
                <HeaderCell>Ticket types failed</HeaderCell>
                <Cell>{rowData => (rowData.ticketTypesFailed && rowData.ticketTypesFailed.length > 0) ? 'yes' : 'no'}</Cell>
            </Column>
            <Column>
                <HeaderCell>Total Events From Service</HeaderCell>
                <Cell>{rowData => rowData.totalEventsFromService ? rowData.totalEventsFromService.toString() : "0"}</Cell>
            </Column>
            <Column>
                <HeaderCell>Events Inserted</HeaderCell>
                <Cell>{rowData => rowData.eventsInserted ? rowData.eventsInserted.toString() : "0"}</Cell>
            </Column>
            <Column>
                <HeaderCell>Events Updated</HeaderCell>
                <Cell>{rowData => rowData.eventsUpdated ? rowData.eventsUpdated.toString() : "0"}</Cell>
            </Column>
            <Column>
                <HeaderCell>Orders Inserted</HeaderCell>
                <Cell>{rowData => rowData.ordersInserted ? rowData.ordersInserted.toString() : "0"}</Cell>
            </Column>
            <Column>
            ...
            <Column>
                ...
                <Cell>{rowData => rowData.ticketsInserted ? rowData.ticketsInserted.toString() : "0"}</Cell>
            </Column>
            <Column>
