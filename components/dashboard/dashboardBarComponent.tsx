...
export default function DashboardBar() {    
    ...
    const exportDashboardData = () => {
        let dashboardSelection = { ...currentDashboardSelection };
        if (!dashboardSelection.currentDashboardData || !dashboardSelection.start || !dashboardSelection.end) {
            return;
        }

        getAllOrders(dashboardSelection.start, dashboardSelection.end)
            .then((response: GetOrdersResponse) => {
                if (response.orders && !response.orderError) {                        
                    if (dashboardSelection.currentDashboardData) {
                        ...
                        const fileName = getFileNameFromDashboardReportSelection('dashboard-orders', dashboardSelection);
                        downloadFile(fileName, csvData);
                        dispatch(
                            setCurrentDashboardData(dashboardSelection.currentDashboardData)
                        );
                    }                        
                }                    
            });
